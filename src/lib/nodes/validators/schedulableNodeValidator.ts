...
export function scheduledNodeValidator(node: BaseNode<unknown>): boolean {
  const parent: BaseNode<SchedulableNodeData> | undefined = <BaseNode<SchedulableNodeData>>(
    node.getParent
  );
  if (typeof parent === 'undefined') return true;

  const isScheduledAtPresent =
    'scheduled_at' in parent.data && 'scheduled_at' in <SchedulableNodeData>node.data;
